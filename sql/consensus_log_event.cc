...
Consensus_log_event::Consensus_log_event(uint32 flag_arg,
  uint64 term_arg, uint64 index_arg, uint64 length_arg)
       : binary_log::Consensus_event (flag_arg, term_arg, index_arg, length_arg),
#ifdef MYSQL_SERVER
       Log_event(header(), footer(), Log_event::EVENT_TRANSACTIONAL_CACHE, Log_event::EVENT_NORMAL_LOGGING)
#else
       Log_event(header(), footer())
#endif
{
  DBUG_ENTER("Consensus_log_event::Consensus_log_event(int, uint64, uint64, uint64)");
...
Consensus_log_event::Consensus_log_event(const char *buffer, uint event_len,
...
#else
void
Consensus_log_event::print(FILE *file __attribute__((unused)), PRINT_EVENT_INFO *print_event_info) const
{
  ...
  if (!print_event_info->short_form)
  {
         ...
         my_b_printf(head, "\tConsensus flag=%u\tterm=%llu\tindex=%llu\tlength=%llu\n",
...
#endif
const char *Previous_consensus_index_log_event::SET_STRING_PREFIX = "##PREV_CONSENSUS_INDEX: ";
Previous_consensus_index_log_event::Previous_consensus_index_log_event(uint64 index_arg)
...
Previous_consensus_index_log_event::Previous_consensus_index_log_event(const char *buffer, uint event_len,
       const Format_description_event *description_event) :
       binary_log::Previous_consensus_index_event(buffer, event_len, description_event),
       Log_event(header(), footer())
{
  DBUG_ENTER("Consensus_log_event::Consensus_log_event(const char *,"
         " uint, const Format_description_log_event *");
...
#ifdef MYSQL_SERVER
uint32 Previous_consensus_index_log_event::write_data_header_to_memory(uchar *buffer)
{
  DBUG_ENTER("Consensus_log_event::write_data_header_to_memory");
  uchar *ptr_buffer = buffer;
  memcpy(ptr_buffer, &index, sizeof(index));
...
bool Previous_consensus_index_log_event::write_data_header(Basic_ostream *ostream)
{
  DBUG_ENTER("Consensus_log_event::write_data_header");
  uchar buffer[POST_HEADER_LENGTH];
  write_data_header_to_memory(buffer);
  DBUG_RETURN(wrapper_my_b_safe_write(ostream, (uchar *)buffer,
         POST_HEADER_LENGTH));
}
#endif
#ifdef MYSQL_SERVER
...
#else
void
Previous_consensus_index_log_event::print(FILE *file __attribute__((unused)), PRINT_EVENT_INFO *print_event_info) const
{
  char buffer[MAX_SET_STRING_LENGTH + 1];
  IO_CACHE *const head = &print_event_info->head_cache;
  if (!print_event_info->short_form)
...
#endif
const char *Consensus_cluster_info_log_event::SET_STRING_INFO_LENGTH = "##CONSENSUS CLUSTER INFO LENGTH: ";
Consensus_cluster_info_log_event::Consensus_cluster_info_log_event(uint32 info_length_arg, char* info_arg)
...
Consensus_cluster_info_log_event::Consensus_cluster_info_log_event(const char *buffer, uint event_len,
  const Format_description_event *description_event) :
  binary_log::Consensus_cluster_info_event(buffer, event_len, description_event),
  Log_event(header(), footer())
{
  DBUG_ENTER("Consensus_log_event::Consensus_log_event(const char *,"
    " uint, const Format_description_log_event *");
...
size_t Consensus_cluster_info_log_event::to_string(char *buffer) const
{
  ...
  strcpy(p, SET_STRING_INFO_LENGTH);
...
#ifdef MYSQL_SERVER
uint32 Consensus_cluster_info_log_event::write_data_header_to_memory(uchar *buffer)
{
  DBUG_ENTER("Consensus_log_event::write_data_header_to_memory");
  uchar *ptr_buffer = buffer;
...
bool Consensus_cluster_info_log_event::write_data_header(Basic_ostream *ostream)
{
  DBUG_ENTER("Consensus_log_event::write_data_header");
  uchar buffer[POST_HEADER_LENGTH];
  write_data_header_to_memory(buffer);
...
#else
void
Consensus_cluster_info_log_event::print(FILE *file __attribute__((unused)), PRINT_EVENT_INFO *print_event_info) const
{
  char buffer[MAX_SET_STRING_LENGTH + 1];
  IO_CACHE *const head = &print_event_info->head_cache;
  if (!print_event_info->short_form)
...
Consensus_empty_log_event::Consensus_empty_log_event(const char *buffer, uint event_len,
  const Format_description_event *description_event) :
...
#else
void
Consensus_empty_log_event::print(FILE *file __attribute__((unused)), PRINT_EVENT_INFO *print_event_info) const
{
  char buffer[MAX_SET_STRING_LENGTH + 1];
  IO_CACHE *const head = &print_event_info->head_cache;
  if (!print_event_info->short_form)
