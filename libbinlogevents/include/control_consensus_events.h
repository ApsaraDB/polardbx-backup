...
public:
  ...
  */
  explicit Consensus_event(unsigned int flag_arg,
      ...
      flag(flag_arg), term(term_arg),
      index(index_arg), length(length_arg),
      reserve(0)
  {}
protected:
...
public:
  Previous_consensus_index_event(const char* buf, unsigned int event_len,
    const Format_description_event *desciption_event);
  /**
  Constructor.
  */
...
protected:
  ...
  unsigned long long int index;                                /** max consensus log index in the previous binlog file */
...
public:
  Consensus_cluster_info_event(const char* buf, unsigned int event_len,
    const Format_description_event *desciption_event);
  /**
  Constructor.
  */
  explicit Consensus_cluster_info_event(unsigned int info_length_arg, char *info_arg)
...
public:
  ...
  Constructor.
  */
  explicit Consensus_empty_event()
    : Binary_log_event(CONSENSUS_EMPTY_EVENT)
  {}
};
